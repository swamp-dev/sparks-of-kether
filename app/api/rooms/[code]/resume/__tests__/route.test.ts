...
function makeRequest(code: string): Request {
  return new Request(`http://localhost/api/rooms/${code}/resume`, {
    method: 'POST',
    headers: { authorization: 'Bearer test-token' },
  });
}

describe('POST /api/rooms/[code]/resume', () => {
  beforeEach(() => {
    roomUpdates = [];
  ...
  it('returns 401 when no bearer token is provided', async () => {
    ...
    const req = new Request('http://localhost/api/rooms/KETHR1/resume', { method: 'POST' });
  ...
  it('returns 409 when the room is not in paused state', async () => {
    roomResponse = {
      data: {
        id: 'room-id',
        code: 'KETHR1',
  ...
  it('returns 200 and updates room state to playing on success', async () => {
    const { POST } = await import('../route');
    const res = await POST(makeRequest('KETHR1'), { params: { code: 'KETHR1' } });
