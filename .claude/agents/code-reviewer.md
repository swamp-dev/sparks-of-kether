...
The verdict line is parsed by `/finish-ticket`'s re-review loop
(word-boundary match; precedence block > rework > fix > ship). Keep
the line clean — one verb, then a short reason. Don't write "no
blockers" inside a fix/block verdict line; word-boundary matching
still trips on ambiguous prose.
