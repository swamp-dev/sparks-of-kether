...
describe('turnReducer — phase transitions', () => {
  ...
  it('end-turn from end → move phase + active player rotates', () => {
    ...
    const result = turnReducer({ state: { ...initial, phase: 'end' } }, { kind: 'end-turn' }, RNG);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.next.state.phase).toBe('move');
    expect(result.value.next.state.activePlayerId).not.toBe(initial.activePlayerId);
  });
  ...
  it('move that is the last arrival at Kether trips the Final Threshold ritual (#345)', () => {
    ...
    const result = turnReducer({ state }, { kind: 'move', pathNumber: 13 }, RNG);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe('turnReducer — prep sub-phase: prep-add-modifier', () => {
  ...
  it('appends a spark-burn with sourcePlayerId without consuming the spark', () => {
    ...
    const player = makePlayer({ id: 'p1', position: 'gevurah', hand: [] });
    const state = makeState({}, { players: [player, ally], activePlayerId: 'p1' });
    const result = turnReducer(
      { state: { ...state, phase: 'challenge', challengeSubPhase: 'prep' } },
      {
    ...
    expect(result.value.next.state.players[1]?.sparksHeld.has('hod')).toBe(true);
  });
  ...
  it('appends an assist-request without affecting ally state', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.next.state.pendingModifiers.assistRequests).toEqual(['p2']);
  });
  ...
  it('caps assistRequests at 2 — third add is rejected', () => {
    ...
    const player = makePlayer({ id: 'p1', position: 'gevurah' });
    const state = makeState({}, { players: [player, allyA, allyB, allyC], activePlayerId: 'p1' });
...
describe('turnReducer — prep sub-phase: prep-remove-modifier', () => {
  ...
  it('removes the first matching assist-request', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.next.state.pendingModifiers.assistRequests).toEqual(['p3']);
  });
...
describe('turnReducer — prep sub-phase: prep-confirm', () => {
  ...
  it('treats a staged card-burn no longer in hand as previously consumed (cumulative retry semantic, #281)', () => {
    ...
    expect(result.value.meta?.dropped.filter((d) => d.kind === 'card-burn')).toEqual([]);
  ...
  it('drops invalid assist-request (ally not at same Sefirah) and surfaces it in meta.dropped', () => {
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.meta?.dropped).toEqual([{ kind: 'assist-request', allyId: 'p2' }]);
  });
...
describe('turnReducer — prep-confirm: burn consumption (#281)', () => {
  ...
  it('does NOT re-consume previously-consumed cards on react-retry → prep-confirm', () => {
    ...
    const c1 = turnReducer(snap, { kind: 'prep-confirm', sefirah: 'gevurah', outcome: FAIL }, RNG);
    ...
    const c2 = turnReducer(snap, { kind: 'prep-confirm', sefirah: 'gevurah', outcome: PASS }, RNG);
...
describe('turnReducer — replace-state event', () => {
  it("replaces state wholesale, including the replacement state's phase machinery", () => {
    ...
    const result = turnReducer(before, { kind: 'replace-state', state: replacement }, RNG);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe('turnReducer — no-active-player guard', () => {
  it.each([
    ...
    const phase: TurnPhase = event.kind === 'move' || event.kind === 'meditate' ? 'move' : 'end';
...
describe('turnReducer — meditate draws 2 cards (capped at HAND_CAP) and stays in move (#503)', () => {
  ...
  it('end-turn with stale pendingDiscard skips the cap-check (#523)', () => {
    ...
    const result = turnReducer({ state: stateWithStaleDiscard }, { kind: 'end-turn' }, RNG);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe('turnReducer — start-of-turn refill on end-turn (#502)', () => {
  ...
  it("refills the new active player's hand toward STARTING_HAND_SIZE", () => {
    ...
    const result = turnReducer({ state: { ...state, phase: 'end' } }, { kind: 'end-turn' }, RNG);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
...
describe('turnReducer — encounter envelope lifecycle (#334)', () => {
  ...
  it('seed is deterministic for the same input state', () => {
    ...
    expect(r1.value.next.state.encounter?.seed).toBe(r2.value.next.state.encounter?.seed);
  });
...
describe('turnReducer — Yesod Dream-Peek seeding (#354)', () => {
  ...
  it('react-retry at Yesod: dreamPillar derived from seed + retryCount (deterministic)', () => {
    ...
    const expected = (['mercy', 'severity', 'balance'] as const)[seededRng(seed + 1).int(0, 2)];
...
describe('turnReducer — new PrepModifier variants (#334)', () => {
  ...
  describe('declare-desire (Netzach)', () => {
    it('add: stages a declare-desire modifier with the named sefirah', () => {
      ...
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      expect(result.value.next.state.pendingModifiers.declareDesires).toEqual(['tiferet']);
    });
  ...
  describe('dream-guess (Yesod)', () => {
    it('add: stages a dream-guess modifier with the named pillar', () => {
      ...
      expect(result.ok).toBe(true);
      if (!result.ok) return;
      expect(result.value.next.state.pendingModifiers.dreamGuesses).toEqual(['mercy']);
    });
...
// ─────────────────────────────────────────────────────────────────────────────
// gift-turn — regular-turn card gifting (#296)
// ─────────────────────────────────────────────────────────────────────────────
...
describe('turnReducer — gift-turn (#296)', () => {
  function makeGiftState(overrides?: {
    ...
    shells?: Partial<GameState['shells']>;
    phase?: TurnPhase;
  }) {
    const giver = makePlayer({ id: 'p1', hand: overrides?.giverHand ?? [1, 2, 3] });
    const recipient = makePlayer({ id: 'p2', hand: overrides?.recipientHand ?? [] });
    return makeState(
      {},
      {
        ...
        shells: overrides?.shells
          ? { ...EMPTY_SHELL_STATE, ...overrides.shells }
          : EMPTY_SHELL_STATE,
      },
    );
  }
  ...
  it('transfers the card from giver to recipient, +1 Illumination', () => {
    ...
    const giver = after.players.find((p) => p.id === 'p1');
    const recv = after.players.find((p) => p.id === 'p2');
    expect(giver?.hand).not.toContain(1);
    expect(recv?.hand).toContain(1);
    expect(after.illumination).toBe(1);
  });

  it('allows multiple gifts per turn (no artificial limit)', () => {
    const state = makeGiftState({ giverHand: [1, 2, 3], recipientHand: [] });
    const r1 = turnReducer({ state }, { kind: 'gift-turn', arcanum: 1, recipientId: 'p2' }, RNG);
    expect(r1.ok).toBe(true);
    if (!r1.ok) return;
    const r2 = turnReducer(
      { state: r1.value.next.state },
      { kind: 'gift-turn', arcanum: 2, recipientId: 'p2' },
      RNG,
    );
    expect(r2.ok).toBe(true);
    if (!r2.ok) return;
    const recv = r2.value.next.state.players.find((p) => p.id === 'p2');
    expect(recv?.hand).toContain(1);
    expect(recv?.hand).toContain(2);
    expect(r2.value.next.state.illumination).toBe(2);
  });
  ...
  it('rejects when recipient does not exist', () => {
    const state = makeGiftState({ giverHand: [1, 2] });
    const result = turnReducer(
      { state },
      { kind: 'gift-turn', arcanum: 1, recipientId: 'p99' },
      RNG,
    );
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it('rejects when phase is not move', () => {
    const state = makeGiftState({ phase: 'challenge' });
    const withChallenge = { ...state, challengeSubPhase: 'prep' as const };
    const result = turnReducer(
      { state: withChallenge },
      { kind: 'gift-turn', arcanum: 1, recipientId: 'p2' },
      RNG,
...
describe('turnReducer — gift-turn-accept-over-cap (#296)', () => {
  it('discards one recipient card and transfers the gift, +1 Illumination', () => {
    ...
    const recvAfter = after.players.find((p) => p.id === 'p2');
    expect(giverAfter?.hand).not.toContain(1);
    expect(recvAfter?.hand).not.toContain(10);
    expect(recvAfter?.hand).toContain(1);
    expect(after.discardPile).toContain(10);
    expect(after.illumination).toBe(1);
    expect(recvAfter?.hand).toHaveLength(HAND_CAP);
  });

  it('rejects when discardArcanum is not in recipient hand', () => {
    ...
    const result = turnReducer(
      { state },
      { kind: 'gift-turn-accept-over-cap', arcanum: 1, recipientId: 'p2', discardArcanum: 99 },
      RNG,
    );
    expect(result.ok).toBe(false);
    if (result.ok) return;
...
describe('turnReducer — refuse-gift-turn (#296)', () => {
  it('+1 Separation, no card movement', () => {
    const giver = makePlayer({ id: 'p1', hand: [1, 2] });
    ...
    if (!result.ok) return;
    const after = result.value.next.state;
    expect(after.separation).toBe(1);
    const giverAfter = after.players.find((p) => p.id === 'p1');
